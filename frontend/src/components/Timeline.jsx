...
export function Timeline({ alerts = [], onAlertClick, selectedAlertId }) {
  const trackRef = useRef(null)
  const [isDragging, setIsDragging] = useState(false)
  const [seekPosition, setSeekPosition] = useState(null) // 0-1

  // Map alert timestamp to position (0..1) across 24h
  const markers = useMemo(() => {
    return alerts
      .filter(a => a.has_recording)
      .map(a => {
